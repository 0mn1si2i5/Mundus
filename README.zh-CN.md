...
- **姓氏观察（Surname Atlas）**：有数字排名时显示来源列出的 rank=1 姓氏；没有数字排名的国家保留来源列出的未排序姓氏，并呈现当地文字、拉丁字母转写和少量人工审阅的中文呈现。数字排名记录会直接标在地球对应国家上，并使用保守边界余量和屏幕空间避让。它使用社区快照，不是统一的官方全球排名；缺失字段会明确显示。
